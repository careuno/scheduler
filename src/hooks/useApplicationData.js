...
const useApplicationData = (initial) => {
  ...
  const bookInterview = (id, interview) => {
    ...
    return (
      axios
        .put(`/api/appointments/${id}`, { interview }) // req.body = {interview} or req.body.interview = {student:'Karen, interviewer:10}
  ...
  const updateSpots = function (state, appointments, id) {
    ...
    const aptIDArray = dayObj.appointments; // [1, 2, 3]

    const arrayOfNullAptForDay = aptIDArray.filter(
      (aptID) => !appointments[aptID].interview
    );
